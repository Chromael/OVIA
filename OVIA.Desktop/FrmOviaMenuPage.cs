...
namespace OVIA.Desktop
{
    ...
    public class FrmOviaMenuPage : Form, IOviaWorkspaceScreen, IOviaWorkspaceLayout
    {
        ...
        public FrmOviaMenuPage(string companyId, string userId, string key, string title, string pathText, string selectedMenu, string descriptionText, string bodyText)
        {
            this.companyId = companyId == null ? string.Empty : companyId;
            this.userId = userId == null ? string.Empty : userId;
            this.workspaceMenuKey = string.IsNullOrWhiteSpace(key) ? "MENU_PAGE" : key.Trim();
            this.workspaceTitle = string.IsNullOrWhiteSpace(title) ? "메뉴" : title.Trim();
            this.workspaceDescription = string.IsNullOrWhiteSpace(descriptionText) ? this.workspaceTitle + " 화면 안내가 아직 등록되지 않았습니다." : descriptionText;
            ...
            this.bodyText = string.IsNullOrWhiteSpace(bodyText) ? this.workspaceDescription : bodyText;
        ...
        public void ApplyWorkspaceLayout()
        {
            ...
            if (contentPanel != null)
            {
                ...
                contentPanel.Height = Math.Max(250, ClientSize.Height - contentPanel.Top - 24);
            }
